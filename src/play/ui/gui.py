...
class Gui(Ui):
    def __init__(self, game):
        ...
        # Buttons
        self.save_button = tk.Button(self.sidebar, text="Save Game", command=self._save_game)
        self.save_button.pack(pady=10, fill="x")
        self.new_game_button = tk.Button(self.sidebar, text="New Game", command=self._reset_game,
                                         bg="#55AAFF", fg="white")
        self.new_game_button.pack(pady=(0, 5), fill="x")
        self.quit_button = tk.Button(self.sidebar, text="Quit", command=self._quit_game,
                                     bg="#FF5555", fg="white")
        ...
        self.play_again_button = tk.Button(self.sidebar, text="Play Again", command=self._reset_game)
    ...
    def _setup_player_stats(self):
        ...
        self.black_name_label = tk.Label(black_frame, text=self.game.black_player.name,
        ...
        self.white_name_label = tk.Label(white_frame, text=self.game.white_player.name,
                                         font=("Arial", 12, "bold"), bg="#EEE")
        ...
        self.white_timer_label = tk.Label(white_frame, text="10:00", font=("Arial", 12), bg="#EEE")
        self.white_timer_label.pack(side="left", padx=(5, 0))

        self.black_timer_label = tk.Label(black_frame, text="10:00", font=("Arial", 12), bg="#EEE")
    ...
    def update_move_list(self, san_move):
        if san_move:
    ...
    def _draw_board(self):
        ...
        # Highlights
        if ENABLE_HIGHLIGHT_LAST_MOVE and self.game.last_move:
        ...
        if ENABLE_HIGHLIGHT_CAPTURE_SQUARE and self.game.capture_square is not None:
    ...
    # ================= Image Loading =================
    def _download_and_load_images(self):
        os.makedirs(self.image_dir, exist_ok=True)
        for sym, code in PIECE_CODES.items():
            path = os.path.join(self.image_dir, f"{code}.png")
            if not os.path.exists(path):
                try:
                    urllib.request.urlretrieve(BASE_URL + f"{code}.png", path)
